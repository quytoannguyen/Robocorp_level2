...
*** Keywords ***
Get and log the value of the local vault secrets using the Get Secret keyword
...
*** Keywords ***
Get and log the value of the vault secrets by getting from JSON file provided in project repository (local_files folder)
    ${credentials_from_vault_file}=	Load Json From File	${CURDIR}${/}local_files${/}vault.json
    Log    ${credentials_from_vault_file}
    
    ${username}=	Get Value From Json	${credentials_from_vault_file}	$..username
    ${password}=	Get Value From Json	${credentials_from_vault_file}	$..password
    ${URL}=	 Get Value From Json	${credentials_from_vault_file}	$..URL
    Log    ${username}
    Log    ${password} 
    Log    ${URL}
...
Choose orders.csv file from user local directory
    ...
    Add file input
    ...    label=Please select the orders.csv file (from folder local_files in this project repository)
    ...    name=fileupload
